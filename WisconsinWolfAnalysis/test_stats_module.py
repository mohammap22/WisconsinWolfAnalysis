...
class UnitTests(unittest.TestCase):
    ...
    def test_correlation_file_format(self):
        ...
        with self.assertRaises(TypeError):
            hypothesis_function_one('./test_files/stats_test_files/' +
                                    'WolfReport2017.pdf')
    ...
    def test_correlation_three_columns(self):
        ...
        with self.assertRaises(TypeError):
            hypothesis_function_one(
                'test_files/stats_test_files/two_col_wolf_and_deer_pop.csv')

    def test_correlation_other_cols_numeric(self):
        """Second and third columns are numeric"""
        with self.assertRaises(TypeError):
            hypothesis_function_one(
                'test_files/stats_test_files/wolf_and_deer_pop_non_num.csv')

    def test_correlation_enough_entries(self):
        """Data frame has at least 2 rows"""
        with self.assertRaises(TypeError):
            hypothesis_function_one(
                'test_files/stats_test_files/wolf_and_deer_pop_one_row.csv')
    ...
    def test_correlation_of_one(self, mock_show):
        ...
        self.assertAlmostEqual(hypothesis_function_one(
            'test_files/stats_test_files/wolf_and_deer_pop_perf_corr.csv'),
            (1.0, 0.0))
    ...
    def test_graph_csv(self):
        ...
        with self.assertRaises(TypeError):
            hypothesis_function_two(
                'test_files/stats_test_files/WolfReport2017.pdf')

    def test_graph_min_two_cols(self):
        """Dataframe has at least 2 columns"""
        with self.assertRaises(TypeError):
            hypothesis_function_two(
                'test_files/stats_test_files/one_col_wolf_and_deer_pop.csv')
    ...
    def test_proportion_file_format(self):  # check!
        """Input file is csv"""
        with self.assertRaises(TypeError):
            hypothesis_function_three('./test_files/stats_test_files/' +
                                      'WolfReport2017.pdf')
